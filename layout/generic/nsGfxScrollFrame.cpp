...
#undef NEEDS_REFLOW
  ...
  if (mHelper.mIsRoot) {
    if (oldScrollAreaBounds.Size() != newScrollAreaBounds.Size()) {
...
a11y::AccType nsHTMLScrollFrame::AccessibleType() {
  ...
  // Create an accessible regardless of focusable state because the state can
  // be changed during frame life cycle without any notifications to
  // accessibility.
 ...
 public:
  ...
  void Simulate(const TimeDuration& aDeltaTime) {
    ...
    if (desired.x != clamped.x) {
      // The scroll has hit the "wall" at the left or right edge of the
      // allowed scroll range. Absorb the impact to avoid bounceback effect.
  ...
  virtual void WillRefresh(mozilla::TimeStamp aTime) override {
    ...
    // We don't access members after returning, so no need for
    // KungFuDeathGrip.
    ScrollFrameHelper::AsyncSmoothMSDScrollCallback(mCallee, deltaTime);
  }

  /*
   * Set a refresh observer for smooth scroll iterations (and start
   * observing). Should be used at most once during the lifetime of this
   * object.
   */
...
class ScrollFrameHelper::AsyncScroll final : public nsARefreshObserver {
 ...
 private:
  ...
  // Bodies of WillRefresh and RefreshDriver contain ScrollFrameHelper
  // specific code.
 public:
  ...
  if (StaticPrefs::layout_css_scroll_behavior_enabled()) {
    if (aMode == ScrollMode::SmoothMsd) {
      ...
      if (!mAsyncSmoothMSDScroll) {
        ...
        // A previous smooth MSD scroll is still in progress, so we just need
        // to update its range and destination.
...
static void AdjustViews(nsIFrame* aFrame) {
  ...
  // Call AdjustViews recursively for all child frames except the popup list
  // as the views for popups are not scrolled.
...
void ScrollFrameHelper::ScrollToImpl(nsPoint aPt, const nsRect& aRange,
  ...
  if (nsLayoutUtils::AsyncPanZoomEnabled(mOuter) &&
    ...
    //    unchanged.
    // 2) If non-APZ triggered this scroll, but we can handle it by just
    // asking
...
static void AppendToTop(nsDisplayListBuilder* aBuilder,
  ...
  if (aFlags & APPEND_OWN_LAYER) {
    ...
    // Build the wrap list with an index of 1, since the scrollbar frame
    // itself might have already built an nsDisplayWrapList.
...
void ScrollFrameHelper::AppendScrollPartsTo(nsDisplayListBuilder* aBuilder,
  ...
  // viewport scrollbars. They would create layerization problems. This
  // wouldn't normally be an issue but themes can add overflow areas to
  // scrollbar parts.
  if (mIsRoot) {
...
void ScrollFrameHelper::BuildDisplayList(nsDisplayListBuilder* aBuilder,
  ...
  if (aBuilder->IsForFrameVisibility()) {
    // We expand the dirty rect to catch frames just outside of the scroll
    // port. We use the dirty rect instead of the whole scroll port to prevent
  ...
  // of the viewport, so most layer implementations would create a layer
  // buffer that's much larger than necessary. Creating independent layers for
  // each scrollbar works around the problem.
  ...
  if (ignoringThisScrollFrame) {
...

    {
      ...
      // Don't clip the scrolled child, and don't paint
      // scrollbars/scrollcorner. The scrolled frame shouldn't have its own
      // background/border, so we can just pass aLists directly.
  ...
  // area to the dispatch to content layer event regions) necessary to
  // activate a scroll frame so it creates a scrollable layer.
  ...
  if (MOZ_UNLIKELY(
    ...
    if ((cbH && (clipRect.width != so.width || so.x < 0)) ||
      ...
      // We prepare an extra DisplayItemClipChain here that will be
      // intersected with those items after they've been created.
      const ActiveScrolledRoot* asr = aBuilder->CurrentActiveScrolledRoot();
  ...
  {
    // Note that setting the current scroll parent id here means that
    // positioned children of this scroll info layer will pick up the scroll
    // info layer as their scroll handoff parent. This is intentional because
    // that is what happens for positioned children of scroll layers, and we
    // want to maintain consistent behaviour between scroll layers and scroll
    // info layers.
...
ScrollStyles ScrollFrameHelper::GetScrollStylesFromFrame() const {
  ...
  if (nsDocShell* ds = nsDocShell::Cast(presContext->GetDocShell())) {
...
void ScrollFrameHelper::ScrollBy(nsIntPoint aDelta, ScrollUnit aUnit,
  ...
  // be responsive without forcing the user to wait for the fling animations
  // to completely stop.
  switch (aMomentum) {
  ...
  if (aSnap == nsIScrollableFrame::ENABLE_SNAP) {
    if (NeedsScrollSnap()) {
      ...
      if (aOrigin == ScrollOrigin::MouseWheel) {
        // When using a clicky scroll wheel, snap point selection works the
        // same as keyboard up/down/left/right navigation, but with varying
        // amounts of scroll delta.
...
void ScrollFrameHelper::ScrollByCSSPixels(
  ...
  // layer-pixels, so starting from such points will make the difference
  // between the given position in script (e.g. scrollTo) and the aligned
  // position larger, in the worst case the difference can be observed in CSS
  // pixels. To avoid it, we use the current position in CSS pixels as the
  // start position.  Hopefully it exactly matches the position where it was
  // given by the previous scrolling operation, but there may be some edge
  // cases where the current position in CSS pixels differs from the given
  // position, the cases should be fixed in bug 1556685.
  CSSIntPoint currentCSSPixels = GetScrollPositionCSSPixels();
...
/**
 * Compute the scrollport size excluding any fixed-pos and sticky-pos (that
 * are stuck) headers and footers. A header or footer is an box that spans
 * that entire width of the viewport and touches the top (or bottom,
 * respectively) of the viewport. We also want to consider fixed/sticky
 * elements that stack or overlap to effectively create a larger header or
 * footer. Headers and footers that cover more than a third of the the
 * viewport are ignored since they probably aren't true headers and footers
 * and we don't want to restrict scrolling too much in such cases. This is a
 * bit conservative --- some pages use elements as headers or footers that
 * don't span the entire width of the viewport --- but it should be a good
 * start.
 *
...
nsresult ScrollFrameHelper::FireScrollPortEvent() {
  ...
  // TODO(emilio): why do we need the whole WillPaintObserver infrastructure
  // and can't use AddScriptRunner & co? I guess it made sense when we used
...
void ScrollFrameHelper::FireScrollEvent() {
  ...
  // refresh driver until it is unsuppressed. The event is marked as delayed
  // so that the refresh driver does not continue ticking.
...
nsRect ScrollFrameHelper::GetScrolledRect() const {
  ...
  // Expand / contract the result by up to half a layer pixel so that
  // scrolling to the right / bottom edge does not change the layer pixel
  // alignment of the scrolled contents.
  ...
  // relative to the reference frame, since that's the space where painting
  // does snapping.
  nsSize visualViewportSize = GetVisualViewportSize();
...
uint32_t ScrollFrameHelper::GetAvailableScrollingDirectionsForUserInputEvents()
  ...
  // We check if there is at least one half of a screen pixel of scroll range
  // to roughly match what apz does when it checks if the change in scroll
  // position in screen pixels round to zero or not.
  ...
  switch (styleDisplay->mScrollSnapAlign.block) {
    ...
    case StyleScrollSnapAlignKeyword::End:
      if (aWritingModeOnScroller.IsVerticalRL()) {
        ...
        // scroll port on horizontal-tb when the frame is positioned at the
        // bottom edge of the scroll port. For this reason we subtract
        // containerBSize from BEnd of the target.
        blockDirectionPosition.emplace(
...
/**
 * Collect the scroll positions corresponding to snap positions of frames in
 * the subtree rooted at |aFrame|, relative to |aScrolledFrame|, into
 * |aSnapInfo|. If |aSnapport| is given, elements outside of the range of
 * |aSnapport| will be ignored.
 */
...
bool ScrollFrameHelper::DragScroll(WidgetEvent* aEvent) {
  // Dragging is allowed while within a 20 pixel border. Note that device
  // pixels are used so that the same margin is used even when zoomed in or
  // out.
...
nsTArray<ScrollPositionUpdate> ScrollFrameHelper::GetScrollUpdates() {
  return std::move(mScrollUpdates);
}

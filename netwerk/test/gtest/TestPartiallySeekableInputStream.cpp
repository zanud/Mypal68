...
TEST(TestPartiallySeekableInputStream, AsyncInputStreamLength)
{
  ...
  nsresult rv =
      qi->AsyncLengthWait(callback, GetCurrentSerialEventTarget());
  ASSERT_EQ(NS_OK, rv);
...
TEST(TestPartiallySeekableInputStream, AbortLengthCallback)
{
  ...
  nsresult rv =
      qi->AsyncLengthWait(callback1, GetCurrentSerialEventTarget());
  ASSERT_EQ(NS_OK, rv);
...
TEST(TestPartiallySeekableInputStream, AsyncWaitAfterConsumed)
{
  ...
  nsresult rv =
      psis->AsyncWait(callback, 0, 0, GetCurrentSerialEventTarget());
  ASSERT_EQ(NS_OK, rv);

...
struct FactoryOp::MaybeBlockedDatabaseInfo final {
  ...
  bool operator==(const Database* aOther) const { return mDatabase == aOther; }
...
class ObjectStoreAddOrPutRequestOp::StoredFileInfo final {
 ...
 public:
  ...
  StoredFileInfo(StoredFileInfo&& aOther)
      : mFileInfo{WrapNotNull(std::move(aOther.mFileInfo))}, mFileActorOrInputStream {
...
nsresult DispatchAndReturnFileReferences(
  ...
  *aDBRefCnt = -1;
  ...
  mozilla::Monitor2 monitor(__func__);
  bool waiting = true;
  ...
  mozilla::Monitor2AutoLock autolock(monitor);
  while (waiting) {
    autolock.Wait();
  }

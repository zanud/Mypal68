...
class SyncedBookmarksMirror {
  ...
  async tryApply(
    ...
    MirrorLog.debug("Merging bookmarks in Rust");
    let wasMerged = await this.merge(signal, localTimeSeconds,
                                     remoteTimeSeconds, weakUpload);
    ...
    MirrorLog.debug("Notifying Places observers");
    try {

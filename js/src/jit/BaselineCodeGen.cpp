...
bool BaselineInterpreterCodeGen::emit_Pick() {
  ...
  Label top, done;
  masm.bind(&top);
  masm.branchSub32(Assembler::Signed, Imm32(1), PCRegAtStart, &done);
  {
...
bool BaselineInterpreterCodeGen::emit_Unpick() {
  ...
  Label top, done;
  masm.bind(&top);
  masm.branchSub32(Assembler::Zero, Imm32(1), PCRegAtStart, &done);
  {
...
bool BaselineInterpreterCodeGen::emit_InitElemArray() {
  ...
  masm.loadPtr(frame.addressOfInterpreterPC(), index);
  LoadInt32Operand(masm, index, index);
...
bool BaselineInterpreterCodeGen::emit_ResumeKind() {
  LoadUint8Operand(masm, PCRegAtStart, R0.scratchReg());
  masm.tagValue(JSVAL_TYPE_INT32, R0.scratchReg(), R0);
  frame.push(R0);
  return true;
}
...
bool BaselineInterpreterGenerator::generate(BaselineInterpreter& interpreter) {
...

  {
    ...
    interpreter.init(
        code, interpretOpOffset_, bailoutPrologueOffset_.offset(),
        profilerEnterFrameToggleOffset_.offset(),
